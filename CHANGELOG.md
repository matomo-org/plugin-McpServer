...
- Changed where `McpServer.serverEvent` publishes tool activity so that `matomo_*` tool calls and tool listings are observed under MCP revision `2026-07-28` as well, not only for clients that opened a session through `initialize`. The event payloads are unchanged, except that a `2026-07-28` request reports no session id — that revision has no session — and names the calling client on the tool-call event instead, which is where its identity now travels. Other requests on that revision are not observable: `server/discover` and `subscriptions/listen` are answered inside the MCP SDK.
