...
class SamplingMessage extends Content
{
     ...
     */
    public function __construct(public readonly Role $role, TextContent|ImageContent|AudioContent|ToolUseContent|ToolResultContent|array $content, public readonly ?array $meta = null)
    {
        if (\is_array($content)) {
            if ([] === $content) {
    ...
    public static function fromArray(array $data) : self
    {
        ...
        return new self($role, $isSingleContent ? $content[0] : $content, isset($data['_meta']) && \is_array($data['_meta']) ? $data['_meta'] : null);
    }
    /**
     * @return array{
     *     role: string,
    ...
    {
        $data = ['role' => $this->role->value, 'content' => $this->content];
        if (null !== $this->meta) {
