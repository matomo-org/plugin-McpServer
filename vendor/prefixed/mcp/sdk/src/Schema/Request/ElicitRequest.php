...
final class ElicitRequest extends Request
{
     ...
     */
    public function __construct(public readonly string $message, public readonly ?ElicitationSchema $requestedSchema = null, public readonly ElicitationMode $mode = ElicitationMode::Form, public readonly ?string $url = null)
    {
    ...
    protected function getParams() : array
    {
        if (ElicitationMode::Url === $this->mode) {
            return ['message' => $this->message, 'mode' => $this->mode->value, 'url' => $this->url];
        }
        // We don't need to send the mode if it's the default (form).
        return ['message' => $this->message, 'requestedSchema' => $this->requestedSchema];
    }
}
