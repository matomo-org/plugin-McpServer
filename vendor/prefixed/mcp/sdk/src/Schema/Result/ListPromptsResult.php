...
class ListPromptsResult implements ResultInterface
{
     ...
     */
    public function __construct(public readonly array $prompts, public readonly ?string $nextCursor = null)
    {
    }
    /**
     * @param array{
    ...
    public static function fromArray(array $data) : self
    {
        ...
        return new self(array_map(static function (mixed $entry) : Prompt {
            if (!\is_array($entry)) {
        ...
        }, $data['prompts']), $data['nextCursor'] ?? null);
    }
    /**
     * @return array{
    ...
    {
        $result = ['prompts' => array_values($this->prompts)];
        if ($this->nextCursor) {
