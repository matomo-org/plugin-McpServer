...
class ListToolsResult implements ResultInterface
{
     ...
     */
    public function __construct(public readonly array $tools, public readonly ?string $nextCursor = null)
    {
    }
    /**
     * @param array{
    ...
    public static function fromArray(array $data) : self
    {
        ...
        return new self(array_map(static function (mixed $entry) : Tool {
            if (!\is_array($entry)) {
        ...
        }, $data['tools']), $data['nextCursor'] ?? null);
    }
    /**
     * @return array{
     *     tools: array<Tool>,
    ...
    {
        $result = ['tools' => array_values($this->tools)];
        if ($this->nextCursor) {
