...
use Matomo\Dependencies\McpServer\Mcp\Client\Handler\Notification\NotificationHandlerInterface;
use Matomo\Dependencies\McpServer\Mcp\Client\Handler\Notification\ProgressNotificationHandler;
use Matomo\Dependencies\McpServer\Mcp\Client\Handler\Request\RequestHandlerInterface;
use Matomo\Dependencies\McpServer\Mcp\Client\State\ClientState;
use Matomo\Dependencies\McpServer\Mcp\Client\State\ClientStateInterface;
use Matomo\Dependencies\McpServer\Mcp\Client\Stateless\HeaderFactory;
use Matomo\Dependencies\McpServer\Mcp\Client\Stateless\InputRequestResolver;
use Matomo\Dependencies\McpServer\Mcp\Client\Stateless\RequestEnvelope;
use Matomo\Dependencies\McpServer\Mcp\Client\Stateless\ToolCatalog;
use Matomo\Dependencies\McpServer\Mcp\Client\Transport\HeaderAwareTransportInterface;
...
use Matomo\Dependencies\McpServer\Mcp\Schema\Request\DiscoverRequest;
...
class Protocol
{
     ...
     */
    public function __construct(private readonly array $requestHandlers = [], array $notificationHandlers = [], ?MessageFactory $messageFactory = null, ?LoggerInterface $logger = null)
    {
        $this->state = new ClientState();
        $this->messageFactory = $messageFactory ?? MessageFactory::make();
        $this->logger = $logger ?? new NullLogger();
        $this->notificationHandlers = [new ProgressNotificationHandler($this->state), ...$notificationHandlers];
    ...
    public function connect(TransportInterface $transport, Configuration $config) : void
    {
        ...
        if ($config->protocolVersion->isModern()) {
            $this->envelope = new RequestEnvelope($config->protocolVersion, $config->capabilities, $config->clientInfo);
    ...
    private function headersFor(string $payload) : array
    {
        ...
        $decoded = json_decode($payload, \true);
        return \is_array($decoded) ? $this->headers->forMessage($decoded, $this->envelope->protocolVersion()) : [];
    }
    /**
    ...
    public function initialize(Configuration $config) : Response|Error
    {
        ...
        $request = new InitializeRequest($offered->value, $config->capabilities, $config->clientInfo);
        ...
        if ($response instanceof Response) {
            ...
            if (null === $negotiated || $negotiated->isModern()) {
                ...
                return Error::forInvalidParams(\sprintf('Server responded with unsupported protocol version "%s". Supported versions: %s.', $counterOffer, implode(', ', array_map(static fn(ProtocolVersion $v): string => $v->value, ProtocolVersion::handshakeVersions()))), $response->id);
            }
            ...
            $this->state->setInitialized(\true);
            $this->sendNotification(new InitializedNotification());
            $this->logger->info('Initialization complete', ['server' => $initResult->serverInfo->name, 'protocolVersion' => $negotiated->value]);
        }
        return $response;
    }
    /**
    ...
    private function discover(Configuration $config) : Response
    {
        ...
        if ($response instanceof Error) {
            $this->logger->info('Server did not answer "server/discover"; continuing without its metadata.', ['code' => $response->code, 'message' => $response->message]);
    ...
    private function readDiscovery(array $result) : void
    {
        ...
        $this->logger->info('Discovery complete', ['supportedVersions' => $result['supportedVersions'] ?? null]);
    }
    /**
    ...
    private function reconcileVersion(mixed $supportedVersions) : void
    {
        ...
        if (\in_array($current->value, $supportedVersions, \true)) {
            return;
        }
        foreach ($supportedVersions as $candidate) {
            ...
            $this->logger->warning('Server does not speak the configured revision; continuing on one it advertises.', ['configured' => $current->value, 'using' => $version->value]);
    ...
    public function request(Request $request, int $timeout, bool $withProgress = \false) : Response|Error
    {
        ...
        for ($attempt = 0; $attempt < self::MAX_ROUND_TRIPS; ++$attempt) {
            ...
            $payload['params'] = [...$payload['params'] ?? [], 'inputResponses' => (object) $this->inputRequests->resolve($asked)];
            ...
            $this->logger->debug('Retrying request with resolved input', ['method' => $payload['method'] ?? null, 'round' => $attempt + 1]);
        }
    ...
    private function withAcceptedVersion(Error $error) : ?ProtocolVersion
    {
        ...
        foreach ($supported as $candidate) {
            ...
            $this->logger->info('Server rejected the offered protocol revision, retrying with one it supports.', ['offered' => $current->value, 'retrying' => $version->value]);
            $this->envelope = $this->envelope->withProtocolVersion($version);
            $this->state->setProtocolVersion($version);
            return $version;
        }
    ...
    private function exchange(array $payload, int $timeout) : Response|Error
    {
        ...
        try {
            ...
            return \Fiber::suspend(['type' => 'await_response', 'request_id' => $requestId, 'timeout' => $timeout]);
        } finally {
    ...
    private function send(array $payload, string $kind) : void
    {
        ...
        $this->logger->debug('Sending ' . $kind, ['id' => $payload['id'] ?? null, 'method' => $payload['method'] ?? null]);
    ...
    private function handleRequest(Request $request) : void
    {
        ...
        $this->logger->debug('Received server request', ['method' => $method, 'id' => $request->getId()]);
        foreach ($this->requestHandlers as $handler) {
            if ($handler->supports($request)) {
                try {
                    $response = $handler->handle($request);
                } catch (\Throwable $e) {
                    $this->logger->error('Unexpected error while handling request', ['method' => $method, 'exception' => $e]);
                    $response = Error::forInternalError(\sprintf('Unexpected error while handling "%s" request', $method), $request->getId());
                }
                $this->sendResponse($response);
                return;
            }
        }
        $error = Error::forMethodNotFound(\sprintf('Client does not handle "%s" requests.', $method), $request->getId());
