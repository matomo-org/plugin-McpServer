 ...
 */
namespace Matomo\Dependencies\McpServer\Mcp\Client\Stateless;
...
final class InputRequestResolver
{
     ...
     */
    private const RESOLVABLE = ['elicitation/create' => ElicitRequest::class, 'sampling/createMessage' => CreateSamplingMessageRequest::class, 'roots/list' => ListRootsRequest::class];
    /**
     * @param RequestHandlerInterface<mixed>[] $handlers
     */
    public function __construct(private readonly array $handlers, private readonly LoggerInterface $logger = new NullLogger())
    {
    }
    /**
    ...
    private function answer(string $key, mixed $ask) : array
    {
        ...
        $request = $class::fromArray(['jsonrpc' => '2.0', 'id' => $key, 'method' => $method, 'params' => \is_array($params) ? $params : null]);
        $this->logger->debug('Resolving multi round-trip input request', ['key' => $key, 'method' => $method]);
    ...
    private function dispatch(Request $request) : array|Error
    {
        foreach ($this->handlers as $handler) {
            ...
            try {
                ...
                $this->logger->error('Input request handler failed', ['method' => $request::getMethod(), 'exception' => $e]);
        ...
        return Error::forMethodNotFound(\sprintf('Client does not handle "%s" requests.', $request::getMethod()), $request->getId());
    }
    /**
