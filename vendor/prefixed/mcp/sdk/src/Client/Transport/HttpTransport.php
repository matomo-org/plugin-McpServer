...
class HttpTransport extends BaseTransport implements HeaderAwareTransportInterface
{
     ...
     */
    public function __construct(private readonly string $endpoint, private readonly array $headers = [], ?ClientInterface $httpClient = null, ?RequestFactoryInterface $requestFactory = null, ?StreamFactoryInterface $streamFactory = null, ?LoggerInterface $logger = null, int $maxSseBufferBytes = self::DEFAULT_MAX_SSE_BUFFER_BYTES)
    {
    ...
    public function close() : void
    {
        if (null !== $this->sessionId) {
            try {
                $request = $this->requestFactory->createRequest('DELETE', $this->endpoint)->withHeader('Mcp-Session-Id', $this->sessionId);
                foreach ($this->headers as $name => $value) {
                    $request = $request->withHeader($name, $value);
                }
    ...
    private function abortSseStream(string $reason) : void
    {
        ...
        $this->sseBuffer = '';
        $this->activeStream = null;
        $this->logger->warning('Aborting SSE stream: ' . $reason, ['session_id' => $this->sessionId, 'buffered_bytes' => $bufferedBytes, 'max_sse_buffer_bytes' => $this->maxSseBufferBytes]);
        if (null === $this->state) {
            return;
        }
    ...
    private function processProgress() : void
    {
        ...
        foreach ($updates as $update) {
            try {
                ($this->activeProgressCallback)($update['progress'], $update['total'], $update['message']);
            } catch (\Throwable $e) {
