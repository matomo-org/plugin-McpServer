...
final class DiscoveryLoader implements LoaderInterface
{
     ...
     */
    public function __construct(private string $basePath, private array $scanDirs, private array $excludeDirs, private DiscovererInterface $discoverer, private array $namePatterns = DiscovererInterface::DEFAULT_NAME_PATERNS, private LoggerInterface $logger = new NullLogger())
    {
    ...
    private function writeDiscovered(RegistryInterface $registry, DiscoveryState $discovered) : DiscoveryState
    {
        ...
        foreach ($discovered->getPrompts() as $name => $reference) {
            ...
            $prompts[$name] = $registry->registerPrompt($reference->prompt, $reference->handler, $reference->completionProviders);
        }
    ...
    private function mayWriteTool(RegistryInterface $registry, string $name) : bool
    {
        ...
        $this->logger->debug(\sprintf('Ignoring discovered tool "%s": a conflicting manual or runtime registration already exists.', $name));
        return \false;
    }
    private function mayWriteResource(RegistryInterface $registry, string $uri) : bool
    {
        ...
        $this->logger->debug(\sprintf('Ignoring discovered resource "%s": a conflicting manual or runtime registration already exists.', $uri));
        return \false;
    }
    private function mayWriteResourceTemplate(RegistryInterface $registry, string $uriTemplate) : bool
    {
        ...
        $this->logger->debug(\sprintf('Ignoring discovered resource template "%s": a conflicting manual or runtime registration already exists.', $uriTemplate));
        return \false;
    }
    private function mayWritePrompt(RegistryInterface $registry, string $name) : bool
    {
        ...
        $this->logger->debug(\sprintf('Ignoring discovered prompt "%s": a conflicting manual or runtime registration already exists.', $name));
        return \false;
    }
}
