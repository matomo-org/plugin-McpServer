 ...
 */
namespace Matomo\Dependencies\McpServer\Mcp\Capability\Formatter;
...
use Matomo\Dependencies\McpServer\Mcp\Schema\Content\PromptMessage;
...
final class PromptResultFormatter
{
    ...
    private function formatContent(mixed $content, ?int $index = null) : TextContent|ImageContent|AudioContent|EmbeddedResource
    {
        ...
        if ($content instanceof Content) {
            if ($content instanceof TextContent || $content instanceof ImageContent || $content instanceof AudioContent || $content instanceof EmbeddedResource) {
                return $content;
            }
    ...
    private function formatResourceContent(array $content, string $indexStr) : EmbeddedResource
    {
        ...
        if (isset($resource['text']) && \is_string($resource['text'])) {
            ...
            $resourceObj = new BlobResourceContents($resource['uri'], $resource['mimeType'] ?? 'application/octet-stream', $resource['blob']);
        } else {
