...
final class CachedDiscoverer implements DiscovererInterface
{
    ...
    public function __construct(private readonly DiscovererInterface $discoverer, private readonly CacheInterface $cache, private readonly LoggerInterface $logger)
    {
    }
    /**
    ...
    public function discover(string $basePath, array $directories, array $excludeDirs = [], array $namePatterns = self::DEFAULT_NAME_PATERNS) : DiscoveryState
    {
        ...
        if (null !== $cachedResult) {
            $this->logger->debug('Using cached discovery results', ['cache_key' => $cacheKey, 'base_path' => $basePath, 'directories' => $directories]);
            return $cachedResult;
        }
    ...
    {
        $keyData = ['base_path' => $basePath, 'directories' => $directories, 'exclude_dirs' => $excludeDirs];
