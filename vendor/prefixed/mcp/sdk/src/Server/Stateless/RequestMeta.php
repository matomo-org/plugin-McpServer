...
final class RequestMeta
{
     ...
     */
    public function __construct(public readonly string $protocolVersion, public readonly ClientCapabilities $clientCapabilities, public readonly ?Implementation $clientInfo = null, public readonly ?LoggingLevel $logLevel = null, public readonly array $traceContext = [])
    {
    }
    /**
    ...
    public static function fromParams(?array $params, array $headers = []) : self
    {
        ...
        return new self($version, ClientCapabilities::fromArray((array) $capabilities), \is_array($clientInfo) ? Implementation::fromArray($clientInfo) : null, self::parseLogLevel($meta[self::LOG_LEVEL] ?? null), self::parseTraceContext($meta, $headers));
    }
    /**
