...
use Matomo\Dependencies\McpServer\Mcp\Schema\Notification\PromptListChangedNotification;
use Matomo\Dependencies\McpServer\Mcp\Schema\Notification\ResourceListChangedNotification;
use Matomo\Dependencies\McpServer\Mcp\Schema\Notification\ResourceUpdatedNotification;
use Matomo\Dependencies\McpServer\Mcp\Schema\Notification\ToolListChangedNotification;
...
final class NotificationFilter
{
     ...
     */
    public function __construct(public readonly bool $toolsListChanged = \false, public readonly bool $promptsListChanged = \false, public readonly bool $resourcesListChanged = \false, public readonly array $resourceSubscriptions = [])
    {
    }
    /**
    ...
    public static function fromParams(?array $notifications) : self
    {
        ...
        return new self(\true === ($notifications['toolsListChanged'] ?? \false), \true === ($notifications['promptsListChanged'] ?? \false), \true === ($notifications['resourcesListChanged'] ?? \false), \is_array($uris) ? array_values(array_filter($uris, \is_string(...))) : []);
    }
    /**
    ...
    {
        return new self($this->toolsListChanged && \true === $capabilities->toolsListChanged, $this->promptsListChanged && \true === $capabilities->promptsListChanged, $this->resourcesListChanged && \true === $capabilities->resourcesListChanged, \true === $capabilities->resourcesSubscribe ? $this->resourceSubscriptions : []);
    }
    /**
    ...
    public function carries(Notification $notification) : bool
    {
        return match (\true) {
            ...
            $notification instanceof ResourceUpdatedNotification => \in_array($notification->uri, $this->resourceSubscriptions, \true),
            default => \false,
        };
    }
    /**
    ...
    public function toAcknowledgedArray() : array
    {
        ...
        if ($this->toolsListChanged) {
            $data['toolsListChanged'] = \true;
        }
        if ($this->promptsListChanged) {
            $data['promptsListChanged'] = \true;
        }
        if ($this->resourcesListChanged) {
            $data['resourcesListChanged'] = \true;
        }
