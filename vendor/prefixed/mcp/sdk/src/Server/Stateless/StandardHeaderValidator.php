...
final class StandardHeaderValidator
{
    ...
    public function __construct(private readonly ?RegistryInterface $registry = null)
    {
    }
    /**
    ...
    {
        return $this->checkMethod($method, $headers) ?? $this->checkName($method, $params, $headers) ?? $this->checkParams($method, $params, $headers);
    }
    /**
     * @param array<string, string> $headers
     */
    ...
    private function checkParams(string $method, ?array $params, array $headers) : ?string
    {
        ...
        foreach (self::mirroredProperties($tool->inputSchema) as $name => $path) {
            $error = $this->checkParam(self::PARAM_HEADER_PREFIX . $name, $headers, self::valueAt($arguments, $path));
            if (null !== $error) {
                return $error;
            }
        }
    ...
    private function checkParam(string $headerName, array $headers, mixed $argument) : ?string
    {
        ...
        if ((\is_int($argument) || \is_float($argument)) && 1 === preg_match('/^-?\\d+(?:\\.\\d+)?$/', $decoded)) {
            return (float) $decoded === (float) $argument ? null : \sprintf('%s header "%s" does not match the body argument "%s".', $headerName, $decoded, $expected);
        }
        if ($decoded !== $expected) {
