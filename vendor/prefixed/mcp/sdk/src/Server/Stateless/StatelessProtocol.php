 ...
 */
namespace Matomo\Dependencies\McpServer\Mcp\Server\Stateless;
...
use Matomo\Dependencies\McpServer\Mcp\Exception\MissingRequestMetaException;
use Matomo\Dependencies\McpServer\Mcp\Exception\MissingRequiredClientCapabilityException;
use Matomo\Dependencies\McpServer\Mcp\Exception\RequestStateException;
...
use Matomo\Dependencies\McpServer\Mcp\Schema\Result\DiscoverResult;
...
use Matomo\Dependencies\McpServer\Mcp\Server\Subscription\NotificationBusInterface;
use Matomo\Dependencies\McpServer\Mcp\Server\Wire\CachePolicy;
use Matomo\Dependencies\McpServer\Mcp\Server\Wire\InboundClassifier;
use Matomo\Dependencies\McpServer\Mcp\Server\Wire\Rev2026Codec;
use Matomo\Dependencies\McpServer\Mcp\Server\Wire\WireCodecInterface;
...
final class StatelessProtocol
{
    ...
    public function handle(string $body, array $headers = []) : StatelessResult
    {
        try {
            ...
            $decoded = json_decode($body, \true, flags: \JSON_THROW_ON_ERROR);
        ...
        if (\in_array($method, self::REMOVED_METHODS, \true)) {
            return StatelessResult::error(Error::forMethodNotFound(\sprintf('Method "%s" does not exist in protocol version %s.', $method, $meta->protocolVersion), $id), 404);
        }
    ...
    private function checkVersion(RequestMeta $meta, array $headers, string|int|null $id) : ?StatelessResult
    {
        ...
        if (null === $headerVersion && $this->requiresTransportHeaders()) {
            return StatelessResult::error(Error::forHeaderMismatch(\sprintf('Missing required MCP-Protocol-Version header (_meta declares "%s").', $meta->protocolVersion), $id), 400);
        }
        ...
        if (null === $version || !\in_array($version, $this->supportedVersions, \true)) {
            return StatelessResult::error(Error::forUnsupportedProtocolVersion($meta->protocolVersion, $this->supportedVersions, $id), 400);
        }
        return null;
    }
    /**
    ...
    private function listen(?array $params, string|int $id) : StatelessResult
    {
        ...
        return StatelessResult::stream(static function () use($agreed, $id, $lifetime, $bus, $codec) : \Generator {
            ...
            (yield ['jsonrpc' => '2.0', 'method' => self::ACKNOWLEDGED_NOTIFICATION, 'params' => ['_meta' => [RequestMeta::SUBSCRIPTION_ID => $id], 'notifications' => (object) $agreed->toAcknowledgedArray()]]);
            ...
            $deadline = 0.0 >= $lifetime ? \INF : microtime(\true) + $lifetime;
            ...
            // dropped transport.
            (yield ['jsonrpc' => '2.0', 'id' => $id, 'result' => $codec->encodeResult(self::LISTEN_METHOD, ['resultType' => 'complete', '_meta' => [RequestMeta::SUBSCRIPTION_ID => $id]], \false)]);
        });
    }
    /**
    ...
    {
        return new DiscoverResult($this->supportedVersions, $this->configuration->capabilities, $this->configuration->instructions);
    }
    /**
    ...
    private function dispatch(string $method, array $decoded, RequestMeta $meta, string|int|null $id, bool $wantsStream = \false) : StatelessResult
    {
        ...
        if ($request instanceof InvalidInputMessageException) {
            ...
            return StatelessResult::error($unknownMethod ? $this->unknownMethod($method, $id) : Error::forInvalidRequest($request->getMessage(), $id), $unknownMethod ? 404 : 400);
        }
        ...
        foreach ($this->requestHandlers as $handler) {
            ...
            try {
                ...
                while ($run->valid()) {
                    $this->logger->debug('Dropped a notification: the client did not accept a response stream.', ['method' => $method, 'notification' => $run->current()::getMethod()]);
                    $run->next();
                }
    ...
    private function unknownMethod(string $method, string|int $id) : Error
    {
        ...
        if (null !== $extension) {
            return Error::forMethodNotFound(\sprintf('Method "%s" belongs to the "%s" extension, which this server does not serve.', $method, $extension), $id);
        }
    ...
    private function streamFrames(\Generator $run, RequestMeta $meta, string $method, string|int $id, bool $cacheable) : \Generator
    {
        ...
        (yield $result instanceof Error ? $result->jsonSerialize() : ['jsonrpc' => '2.0', 'id' => $id, 'result' => $this->codec->encodeResult($method, (array) $result->result->jsonSerialize(), $cacheable)]);
    }
    /**
    ...
    private function checkInputRequests(ResultInterface $result, RequestMeta $meta, string $method, string|int $id) : ?StatelessResult
    {
        ...
        $this->logger->warning('A handler asked for input the client did not declare it could provide; the ask was replaced with -32021.', ['method' => $method, 'required' => $missing->jsonSerialize()]);
        return StatelessResult::error(Error::forMissingRequiredClientCapability('The server needs input this client did not declare it can provide.', $missing, $id), 400);
    }
    /**
    ...
    private function toErrorResult(string $method, string|int $id, \Throwable $e) : StatelessResult
    {
        if ($e instanceof MissingRequiredClientCapabilityException) {
            return StatelessResult::error(Error::forMissingRequiredClientCapability($e->getMessage(), $e->requiredCapabilities, $id), 400);
        }
     ...
     */
    private function encode(string $method, string|int $id, ResultInterface $result, bool $cacheable = \true) : StatelessResult
    {
