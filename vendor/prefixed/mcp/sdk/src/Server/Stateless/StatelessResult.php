...
final class StatelessResult
{
     ...
     */
    private function __construct(public readonly ?MessageInterface $message, public readonly int $httpStatus, public readonly ?\Closure $frames = null, private readonly ?array $body = null, private readonly string|int|null $id = null, private readonly bool $bodyless = \false)
    {
    }
    /**
    ...
    {
        return new self(null, $httpStatus, bodyless: \true);
    }
    /**
    ...
    {
        if (null !== $this->body) {
            return json_encode(['jsonrpc' => MessageInterface::JSONRPC_VERSION, 'id' => $this->id, 'result' => $this->body], \JSON_THROW_ON_ERROR | \JSON_UNESCAPED_SLASHES);
        }
        if ($this->bodyless) {
