...
use Matomo\Dependencies\McpServer\Mcp\Schema\Extension\AbstractExtension;
use Matomo\Dependencies\McpServer\Mcp\Schema\Extension\ExtensionInterface;
...
use Matomo\Dependencies\McpServer\Mcp\Server\Stateless\StandardHeaderValidator;
use Matomo\Dependencies\McpServer\Mcp\Server\Stateless\StatelessProtocol;
use Matomo\Dependencies\McpServer\Mcp\Server\Subscription\InMemoryNotificationBus;
use Matomo\Dependencies\McpServer\Mcp\Server\Subscription\NotificationBusInterface;
use Matomo\Dependencies\McpServer\Mcp\Server\Subscription\Psr16NotificationBus;
use Matomo\Dependencies\McpServer\Mcp\Server\Subscription\PublishingEventDispatcher;
...
final class Builder
{
     ...
     */
    public function setServerInfo(string $name, string $version, ?string $description = null, ?array $icons = null, ?string $websiteUrl = null, ?string $title = null) : self
    {
     ...
     */
    public function setHeaderValidator(bool $headerValidation = \true) : self
    {
    ...
    {
        return null !== $this->requestStateKey ? new RequestStateCodec($this->requestStateKey, $this->requestStateTtl) : null;
    }
    /**
    ...
    {
        $parts = $this->assemble();
        $protocol = new Protocol(requestHandlers: $parts['requestHandlers'], notificationHandlers: $parts['notificationHandlers'], messageFactory: $parts['messageFactory'], sessionManager: $parts['sessionManager'], logger: $parts['logger'], eventDispatcher: $parts['eventDispatcher'], inputRequiredShim: $this->inputRequiredShim ? new InputRequiredShim($this->inputRequiredRounds, $this->inputRequiredTimeout, $parts['logger']) : null, requestStateCodec: $this->requestStateCodec());
        $modernVersions = $this->modernVersions ?? ProtocolVersion::modernVersions();
        return new Server($protocol, $parts['logger'], [] === $modernVersions ? null : $this->buildStateless($modernVersions));
    }
    /**
    ...
    {
        $parts = $this->assemble();
        return new StatelessProtocol(requestHandlers: $parts['requestHandlers'], messageFactory: $parts['messageFactory'], configuration: $parts['configuration'], supportedVersions: $supportedVersions, logger: $parts['logger'], subscriptionLifetime: $this->subscriptionLifetime, headerValidator: $this->headerValidation ? new StandardHeaderValidator($parts['registry']) : null, requestStateCodec: $this->requestStateCodec(), cachePolicy: $this->cachePolicy, notificationBus: $this->notificationBus, extensionMethods: $this->extensionMethods);
    }
    /**
    ...
    private function resolve() : array
    {
        ...
        $eventDispatcher = null !== $this->notificationBus ? new PublishingEventDispatcher($this->notificationBus, $this->eventDispatcher) : $this->eventDispatcher;
        ...
        if (null !== $this->protocolVersion && $this->protocolVersion->isModern()) {
            $logger->warning('Configured protocol version cannot be reached through the "initialize" handshake, negotiating the handshake revisions instead.', ['configured' => $this->protocolVersion->value, 'negotiable' => array_map(static fn(ProtocolVersion $v): string => $v->value, ProtocolVersion::handshakeVersions())]);
        }
