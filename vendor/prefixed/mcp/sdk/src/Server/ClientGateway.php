...
use Matomo\Dependencies\McpServer\Mcp\Exception\ClientException;
...
use Matomo\Dependencies\McpServer\Mcp\Schema\Content\AudioContent;
...
use Matomo\Dependencies\McpServer\Mcp\Schema\Extension\ExtensionIdentifier;
...
use Matomo\Dependencies\McpServer\Mcp\Schema\ModelPreferences;
...
use Matomo\Dependencies\McpServer\Mcp\Schema\Request\ListRootsRequest;
...
use Matomo\Dependencies\McpServer\Mcp\Schema\Result\ListRootsResult;
use Matomo\Dependencies\McpServer\Mcp\Schema\Tool;
use Matomo\Dependencies\McpServer\Mcp\Schema\ToolChoice;
...
class ClientGateway
{
    ...
    {
        \Fiber::suspend(['type' => 'notification', 'notification' => $notification, 'session_id' => $this->session->getId()->toRfc4122()]);
    }
    /**
    ...
    public function sample(array|Content|string $message, int $maxTokens = 1000, int $timeout = 120, array $options = []) : CreateSamplingMessageResult
    {
        ...
        if (\is_object($message) && \in_array($message::class, [TextContent::class, AudioContent::class, ImageContent::class], \true)) {
            $message = [new SamplingMessage(Role::User, $message)];
        }
        $request = new CreateSamplingMessageRequest(messages: $message, maxTokens: $maxTokens, preferences: $preferences, systemPrompt: $options['systemPrompt'] ?? null, includeContext: $options['includeContext'] ?? null, temperature: $options['temperature'] ?? null, stopSequences: $options['stopSequences'] ?? null, metadata: $options['metadata'] ?? null, tools: $options['tools'] ?? null, toolChoice: $options['toolChoice'] ?? null);
    ...
    {
        $response = \Fiber::suspend(['type' => 'request', 'request' => $request, 'session_id' => $this->session->getId()->toRfc4122(), 'timeout' => $timeout, 'input_key' => $key]);
