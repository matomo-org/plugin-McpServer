...
use Matomo\Dependencies\McpServer\Mcp\Server\Stateless\InputRequestCapabilities;
...
final class InputRequiredShim
{
    ...
    public function __construct(private readonly int $maxRounds = self::DEFAULT_MAX_ROUNDS, private readonly int $roundTimeout = self::DEFAULT_ROUND_TIMEOUT, private readonly LoggerInterface $logger = new NullLogger())
    {
    }
    /**
     ...
     */
    public function fulfill(Response|Error $result, RequestHandlerInterface $handler, Request $request, SessionInterface $session, ?RequestStateCodec $codec) : Response|Error
    {
        $round = 0;
        while (($ask = self::askOf($result)) instanceof InputRequiredResult) {
            if (++$round > $this->maxRounds) {
                $this->logger->warning('A handler kept asking for input past the round limit; the call was failed instead.', ['method' => $request::getMethod(), 'rounds' => $this->maxRounds]);
                return Error::forInternalError(\sprintf('The server asked for input more than %d times without reaching a result.', $this->maxRounds), $request->getId());
            }
            if (null !== ($refusal = $this->refuseUndeclared($ask, $request, $session))) {
                return $refusal;
            }
            try {
                $session->set(InputContext::class, new InputContext($this->collect($ask, $session), self::payloadOf($ask, $codec)));
    ...
    private function collect(InputRequiredResult $ask, SessionInterface $session) : array
    {
        ...
        foreach ($ask->inputRequests as $key => $embedded) {
            ...
            if ($answer instanceof Error) {
                ...
                $this->logger->info('The client failed an input request; the handler is re-entered without it.', ['key' => $key, 'error' => $answer->message]);
                continue;
            }
    ...
    private function refuseUndeclared(InputRequiredResult $ask, Request $request, SessionInterface $session) : ?Error
    {
        ...
        $this->logger->warning('A handler asked for input the client did not declare it could provide; the ask was replaced with -32021.', ['method' => $request::getMethod(), 'required' => $missing->jsonSerialize()]);
        return Error::forMissingRequiredClientCapability('The server needs input this client did not declare it can provide.', $missing, $request->getId());
    }
    /**
    ...
    {
        return $result instanceof Response && $result->result instanceof InputRequiredResult ? $result->result : null;
    }
    /**
