...
use Matomo\Dependencies\McpServer\Mcp\Server\Transport\Stdio\RunnerControl;
use Matomo\Dependencies\McpServer\Mcp\Server\Transport\Stdio\RunnerControlInterface;
use Matomo\Dependencies\McpServer\Mcp\Server\Transport\Stdio\RunnerState;
...
class StdioTransport extends BaseTransport
{
     ...
     *                               instead.
     */
    public function __construct(private $input = \STDIN, private $output = \STDOUT, ?LoggerInterface $logger = null, private readonly RunnerControlInterface $runnerControl = new RunnerControl(), private readonly int $maxLineBytes = self::DEFAULT_MAX_LINE_BYTES)
    {
    ...
    public function listen() : int
    {
        ...
        if (\in_array($this->runnerControl->getState(), [RunnerState::RUNNING, RunnerState::STOP_AND_END_SESSION], \true)) {
    ...
    protected function processInput() : void
    {
        ...
        if (!$lineComplete && \strlen($line) >= $this->maxLineBytes - 1) {
            $this->discardingLine = \true;
            $this->logger->warning('StdioTransport discarded an input line exceeding the maximum length.', ['max_line_bytes' => $this->maxLineBytes]);
            return;
        }
