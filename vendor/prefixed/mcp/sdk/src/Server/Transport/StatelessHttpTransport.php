...
use Matomo\Dependencies\McpServer\Mcp\Server\Transport\Http\StatelessResponder;
...
final class StatelessHttpTransport
{
     ...
     */
    public function __construct(private readonly StatelessProtocol $protocol, ?ResponseFactoryInterface $responseFactory = null, ?StreamFactoryInterface $streamFactory = null, private readonly LoggerInterface $logger = new NullLogger(), private readonly int $maxBodyBytes = self::DEFAULT_MAX_BODY_BYTES, ?iterable $middleware = null)
    {
        $this->middleware = null === $middleware ? self::defaultMiddleware() : array_values([...$middleware]);
    ...
    {
        return [new CorsMiddleware(), new DnsRebindingProtectionMiddleware()];
    }
    public function handle(ServerRequestInterface $request) : ResponseInterface
    {
        $handler = new MiddlewareRequestHandler($this->middleware, \Closure::fromCallable([$this, 'dispatch']));
    ...
    private function dispatch(ServerRequestInterface $request) : ResponseInterface
    {
        ...
        if ('POST' !== $request->getMethod()) {
            return $this->json(json_encode(Error::forInvalidRequest(\sprintf('The modern lifecycle accepts POST only, got %s.', $request->getMethod())), \JSON_THROW_ON_ERROR), 405);
        }
        ...
        if (null === $payload) {
            ...
            return $this->json(json_encode(Error::forInvalidRequest(\sprintf('Request body exceeds the maximum allowed size of %d bytes.', $this->maxBodyBytes)), \JSON_THROW_ON_ERROR), 413);
        }
        $headers = [];
