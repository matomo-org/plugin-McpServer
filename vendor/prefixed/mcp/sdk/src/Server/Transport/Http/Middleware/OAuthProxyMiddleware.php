...
use Matomo\Dependencies\McpServer\Http\Discovery\Psr18ClientDiscovery;
use Matomo\Dependencies\McpServer\Mcp\Exception\RuntimeException;
use Matomo\Dependencies\McpServer\Mcp\Server\Transport\Http\OAuth\OidcDiscoveryInterface;
use Matomo\Dependencies\McpServer\Psr\Http\Client\ClientExceptionInterface;
...
final class OAuthProxyMiddleware implements MiddlewareInterface
{
    ...
    private function handleAuthorize(ServerRequestInterface $request) : ResponseInterface
    {
        ...
        return $this->responseFactory->createResponse(302)->withHeader('Location', $upstreamUrl)->withHeader('Cache-Control', 'no-store');
    }
    private function handleToken(ServerRequestInterface $request) : ResponseInterface
    {
        ...
        $upstreamRequest = $this->requestFactory->createRequest('POST', $tokenEndpoint)->withHeader('Content-Type', 'application/x-www-form-urlencoded')->withBody($this->streamFactory->createStream($body));
        ...
        try {
            ...
            return $this->responseFactory->createResponse($upstreamResponse->getStatusCode())->withHeader('Content-Type', $upstreamResponse->getHeaderLine('Content-Type'))->withHeader('Cache-Control', 'no-store')->withBody($this->streamFactory->createStream($responseBody));
    ...
    private function createAuthServerMetadataResponse() : ResponseInterface
    {
        ...
        $localMetadata = ['issuer' => $localBaseUrl, 'authorization_endpoint' => $localBaseUrl . '/authorize', 'token_endpoint' => $localBaseUrl . '/token', 'response_types_supported' => $upstreamMetadata['response_types_supported'] ?? ['code'], 'grant_types_supported' => $upstreamMetadata['grant_types_supported'] ?? ['authorization_code', 'refresh_token'], 'code_challenge_methods_supported' => $upstreamMetadata['code_challenge_methods_supported'] ?? ['S256']];
        $copyFields = ['scopes_supported', 'token_endpoint_auth_methods_supported', 'jwks_uri'];
        foreach ($copyFields as $field) {
            if (isset($upstreamMetadata[$field])) {
                $localMetadata[$field] = $upstreamMetadata[$field];
            }
        }
        return $this->responseFactory->createResponse(200)->withHeader('Content-Type', 'application/json')->withHeader('Cache-Control', 'max-age=3600')->withBody($this->streamFactory->createStream(json_encode($localMetadata, \JSON_THROW_ON_ERROR | \JSON_UNESCAPED_SLASHES)));
    }
    private function createErrorResponse(int $status, string $message) : ResponseInterface
    {
        ...
        return $this->responseFactory->createResponse($status)->withHeader('Content-Type', 'application/json')->withBody($this->streamFactory->createStream($body));
    }
    private function resolveTokenEndpointAuthMethod() : string
    {
        $supportedMethods = $this->getTokenEndpointAuthMethods();
        if (\in_array(self::CLIENT_SECRET_BASIC, $supportedMethods, \true)) {
            return self::CLIENT_SECRET_BASIC;
        }
        if (\in_array(self::CLIENT_SECRET_POST, $supportedMethods, \true)) {
