...
use Matomo\Dependencies\McpServer\Mcp\Server\Stateless\StatelessResult;
use Matomo\Dependencies\McpServer\Mcp\Server\Transport\CallbackStream;
...
{
    public function __construct(private readonly ResponseFactoryInterface $responseFactory, private readonly StreamFactoryInterface $streamFactory, private readonly LoggerInterface $logger = new NullLogger())
    {
    }
    ...
    {
        return $this->responseFactory->createResponse($status)->withHeader('Content-Type', 'application/json')->withBody($this->streamFactory->createStream($payload));
    }
    /**
    ...
    private function sse(\Closure $frames) : ResponseInterface
    {
        ...
        return $this->responseFactory->createResponse(200)->withHeader('Content-Type', 'text/event-stream')->withHeader('Cache-Control', 'no-cache')->withHeader('Connection', 'keep-alive')->withHeader('X-Accel-Buffering', 'no')->withBody(new CallbackStream($callback, $this->logger));
    }
}
