...
use Matomo\Dependencies\McpServer\Mcp\Exception\ClientRegistrationException;
use Matomo\Dependencies\McpServer\Mcp\Exception\InvalidArgumentException;
use Matomo\Dependencies\McpServer\Mcp\Server\Transport\Http\OAuth\ClientRegistrarInterface;
...
final class ClientRegistrationMiddleware implements MiddlewareInterface
{
    ...
    public function __construct(private readonly ClientRegistrarInterface $registrar, private readonly string $localBaseUrl, ?ResponseFactoryInterface $responseFactory = null, ?StreamFactoryInterface $streamFactory = null)
    {
    ...
    private function handleRegistration(ServerRequestInterface $request) : ResponseInterface
    {
        ...
        if (!str_starts_with(strtolower($contentType), 'application/json')) {
            return $this->jsonResponse(400, ['error' => 'invalid_client_metadata', 'error_description' => 'Content-Type must be application/json.'], ['Cache-Control' => 'no-store']);
        }
        $body = $request->getBody()->__toString();
        try {
            $decoded = json_decode($body, \false, 512, \JSON_THROW_ON_ERROR);
        } catch (\JsonException) {
            return $this->jsonResponse(400, ['error' => 'invalid_client_metadata', 'error_description' => 'Request body must be valid JSON.'], ['Cache-Control' => 'no-store']);
        }
        if (!$decoded instanceof \stdClass) {
            return $this->jsonResponse(400, ['error' => 'invalid_client_metadata', 'error_description' => 'Request body must be a JSON object.'], ['Cache-Control' => 'no-store']);
        }
        ...
        try {
            ...
            return $this->jsonResponse(400, ['error' => $e->errorCode, 'error_description' => $e->getMessage()], ['Cache-Control' => 'no-store']);
        }
        return $this->jsonResponse(201, $result, ['Cache-Control' => 'no-store']);
    }
    private function enrichAuthServerMetadata(ResponseInterface $response) : ResponseInterface
    {
        ...
        try {
            $metadata = json_decode($stream->__toString(), \true, 512, \JSON_THROW_ON_ERROR);
        } catch (\JsonException) {
            if ($stream->isSeekable()) {
                $stream->rewind();
            }
        ...
        return $response->withBody($this->streamFactory->createStream(json_encode($metadata, \JSON_THROW_ON_ERROR | \JSON_UNESCAPED_SLASHES)))->withHeader('Content-Type', 'application/json')->withoutHeader('Content-Length');
    }
    /**
    ...
    {
        $response = $this->responseFactory->createResponse($status)->withHeader('Content-Type', 'application/json')->withBody($this->streamFactory->createStream(json_encode($data, \JSON_THROW_ON_ERROR | \JSON_UNESCAPED_SLASHES)));
