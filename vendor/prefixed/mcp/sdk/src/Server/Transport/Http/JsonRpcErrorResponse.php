...
{
    public static function create(ResponseFactoryInterface $responseFactory, StreamFactoryInterface $streamFactory, int $statusCode, Error $error) : ResponseInterface
    {
        $body = json_encode($error, \JSON_THROW_ON_ERROR);
        return $responseFactory->createResponse($statusCode)->withHeader('Content-Type', 'application/json')->withBody($streamFactory->createStream($body));
    }
}
