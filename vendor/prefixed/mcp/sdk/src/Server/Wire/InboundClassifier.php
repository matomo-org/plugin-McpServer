...
final class InboundClassifier
{
    ...
    private function classifyBatch(array $messages, ?string $headerVersion) : EraClassification
    {
        foreach ($messages as $message) {
            ...
            if ($classification->modern) {
                return EraClassification::reject(Error::forInvalidRequest(\sprintf('Protocol revision %s removed JSON-RPC batching; send one message per request.', $classification->claimedVersion)), 400);
            }
        }
        return EraClassification::legacy();
    }
    ...
    private function classifyMessage(array $message, ?string $headerVersion) : EraClassification
    {
        ...
        if (null !== $claim) {
            if (!\is_string($claim) || '' === $claim) {
                return EraClassification::reject(Error::forInvalidParams(\sprintf('Request "_meta" member "%s" must be a non-empty string.', RequestMeta::PROTOCOL_VERSION), $id), 400);
            }
        ...
        return EraClassification::reject(Error::forInvalidParams(\sprintf('Protocol revision %s requires the "%s" member in "params._meta".', $headerVersion, RequestMeta::PROTOCOL_VERSION), $id), 400);
    }
    /**
    ...
    {
        return null !== $version && \true === ProtocolVersion::tryFrom($version)?->isModern();
    }
}
