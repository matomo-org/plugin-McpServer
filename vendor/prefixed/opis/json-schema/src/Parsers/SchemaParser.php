 ...
 * ============================================================================ */
namespace Matomo\Dependencies\McpServer\Opis\JsonSchema\Parsers;
...
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Schema;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Uri;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Schemas\BooleanSchema;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Schemas\EmptySchema;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Schemas\ExceptionSchema;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Schemas\ObjectSchema;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Resolvers\FilterResolver;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Resolvers\FormatResolver;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Resolvers\ContentMediaTypeResolver;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Resolvers\ContentEncodingResolver;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Parsers\Drafts\Draft06;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Parsers\Drafts\Draft07;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Parsers\Drafts\Draft201909;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Parsers\Drafts\Draft202012;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Exceptions\ParseException;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Exceptions\SchemaException;
...
class SchemaParser
{
    ...
    /** @var array */
    protected const DEFAULT_OPTIONS = ['allowFilters' => \true, 'allowFormats' => \true, 'allowMappers' => \true, 'allowTemplates' => \true, 'allowGlobals' => \true, 'allowDefaults' => \true, 'allowSlots' => \true, 'allowKeywordValidators' => \true, 'allowPragmas' => \true, 'allowDataKeyword' => \true, 'allowKeywordsAlongsideRef' => \false, 'allowUnevaluated' => \true, 'allowRelativeJsonPointerInRef' => \true, 'allowExclusiveMinMaxAsBool' => \true, 'keepDependenciesKeyword' => \true, 'keepAdditionalItemsKeyword' => \true, 'decodeContent' => ['06', '07'], 'defaultDraft' => self::DEFAULT_DRAFT, 'varRefKey' => '$ref', 'varEachKey' => '$each', 'varDefaultKey' => 'default'];
    /** @var array */
     ...
     */
    public function __construct(array $resolvers = [], array $options = [], ?Vocabulary $extraVocabulary = null)
    {
        if ($options) {
    ...
    {
        return ['06' => new Draft06($extraVocabulary), '07' => new Draft07($extraVocabulary), '2019-09' => new Draft201909($extraVocabulary), '2020-12' => new Draft202012($extraVocabulary)];
    }
    /**
     * @param array $resolvers
     * @return array
    ...
    {
        if (!property_exists($schema, '$anchor') || !isset($this->drafts[$draft]) || !$this->drafts[$draft]->supportsAnchorId()) {
            return null;
        }
     ...
     * @return Schema|null
     */
    public function parseRootSchema(object $schema, Uri $id, callable $handle_id, callable $handle_object, ?string $draft = null) : ?Schema
    {
        $existent = \false;
        if (property_exists($schema, '$id')) {
            $existent = \true;
            $id = Uri::parse($schema->{'$id'}, \true);
        }
        if ($id instanceof Uri) {
            if ($id->fragment() === null) {
                $id = Uri::merge($id, null, \true);
            }
        } else {
