 ...
 * ============================================================================ */
namespace Matomo\Dependencies\McpServer\Opis\JsonSchema\Parsers\Pragmas;

use Matomo\Dependencies\McpServer\Opis\JsonSchema\Pragma;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Pragmas\SlotsPragma;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Info\SchemaInfo;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Parsers\PragmaParser;
...
class SlotsPragmaParser extends PragmaParser
{
    ...
    public function parse(SchemaInfo $info, SchemaParser $parser, object $shared) : ?Pragma
    {
        ...
        foreach ($value as $name => $slot) {
            ...
            if (is_bool($slot)) {
                $list[$name] = $parser->parseSchema(new SchemaInfo($slot, null, $info->base(), $info->root(), array_merge($info->path(), [$this->pragma, $name]), $info->draft() ?? $parser->defaultDraftVersion()));
