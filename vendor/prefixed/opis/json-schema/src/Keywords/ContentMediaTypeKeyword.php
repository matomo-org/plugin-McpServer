...
use Matomo\Dependencies\McpServer\Opis\JsonSchema\ContentMediaType;
...
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Exceptions\UnresolvedContentMediaTypeException;
class ContentMediaTypeKeyword implements Keyword
{
    ...
    public function validate(ValidationContext $context, Schema $schema) : ?ValidationError
    {
        ...
        $ok = $this->media instanceof ContentMediaType ? $this->media->validate($data, $this->name) : ($this->media)($data, $this->name);
        if ($ok) {
            return null;
        }
        unset($data);
        return $this->error($schema, $context, 'contentMediaType', "The media type of the data must be '{media}'", ['media' => $this->name]);
    }
}
