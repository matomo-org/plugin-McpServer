    ...
    public function validate(ValidationContext $context, Schema $schema) : ?ValidationError
    {
        ...
        foreach ($this->value as $name => $value) {
            ...
            if ($value === \false) {
                ...
                return $this->error($schema, $context, 'dependentSchemas', "'{$name}' property is not allowed", ['property' => $name]);
            }
            ...
            if ($error = $context->validateSchemaWithoutEvaluated($value, null, \false, $object)) {
                $this->addEvaluatedFromArrayObject($object, $context);
                return $this->error($schema, $context, 'dependentSchemas', "The object must match dependency schema defined on property '{$name}'", ['property' => $name], $error);
            }
        }
        $this->addEvaluatedFromArrayObject($object, $context);
        return null;
