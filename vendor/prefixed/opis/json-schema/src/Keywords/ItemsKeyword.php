...
class ItemsKeyword implements Keyword
{
    ...
    {
        if ($this->alwaysValid || $this->value === \true) {
            if ($this->count === -1) {
        ...
        if ($this->count >= 0) {
            ...
            for ($i = $this->startIndex; $i < $max; $i++) {
                ...
                if ($this->value[$i] === \false) {
                    ...
                    return $this->error($schema, $context, $this->keyword, "Array item at index {index} is not allowed", ['index' => $i]);
                }
            ...
            return $this->error($schema, $context, $this->keyword, 'Array items must match corresponding schemas', [], $errors);
        }
        ...
        $error = $this->validateIterableData($schema, $this->value, $context, $this->indexes($this->startIndex, $count), $this->keyword, 'All array items must match schema', [], $object);
