...
use Matomo\Dependencies\McpServer\Opis\JsonSchema\ContentEncoding;
...
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Exceptions\UnresolvedContentEncodingException;
class ContentEncodingKeyword implements Keyword
{
    ...
    public function validate(ValidationContext $context, Schema $schema) : ?ValidationError
    {
        ...
        $result = $this->encoding instanceof ContentEncoding ? $this->encoding->decode($context->currentData(), $this->name) : ($this->encoding)($context->currentData(), $this->name);
        if ($result === null) {
            return $this->error($schema, $context, 'contentEncoding', "The value must be encoded as '{encoding}'", ['encoding' => $this->name]);
        }
