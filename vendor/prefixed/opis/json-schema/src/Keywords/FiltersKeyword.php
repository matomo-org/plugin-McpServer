...
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Errors\CustomError;
use Matomo\Dependencies\McpServer\Opis\JsonSchema\Exceptions\UnresolvedFilterException;
    ...
    public function validate(ValidationContext $context, Schema $schema) : ?ValidationError
    {
        ...
        foreach ($this->filters as $filter) {
            ...
            try {
                ...
                return $this->error($schema, $context, '$filters', $error->getMessage(), $error->getArgs() + ['filter' => $filter->name, 'type' => $type, 'args' => $args]);
            }
            if ($ok) {
                unset($func, $args, $ok);
                continue;
            }
            return $this->error($schema, $context, '$filters', "Filter '{filter}' ({type}) was not passed", ['filter' => $filter->name, 'type' => $type, 'args' => $args]);
        }
        return null;
    }
}
