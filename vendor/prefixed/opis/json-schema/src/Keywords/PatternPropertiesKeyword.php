        ...
        foreach ($this->value as $pattern => $value) {
            ...
            if ($value === \false) {
                ...
                if ($list) {
                    ...
                    return $this->error($schema, $context, 'patternProperties', "Object properties that match pattern '{pattern}' are not allowed", ['pattern' => $pattern, 'forbidden' => $list]);
                }
                unset($list);
                continue;
            }
            ...
            if (!$subErrors->isEmpty()) {
                ...
                return $this->error($schema, $context, 'patternProperties', "Object properties that match pattern '{pattern}' must also match pattern's schema", ['pattern' => $pattern], $subErrors);
            }
            unset($subErrors);
        }
        if ($checked) {
