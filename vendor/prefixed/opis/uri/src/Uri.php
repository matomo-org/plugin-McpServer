 ...
 * ============================================================================ */
namespace Matomo\Dependencies\McpServer\Opis\Uri;
...
class Uri
{
    ...
    {
        $this->components = $components + ['scheme' => null, 'user' => null, 'pass' => null, 'host' => null, 'port' => null, 'path' => null, 'query' => null, 'fragment' => null];
    }
    /**
     * @return string|null
     */
     ...
     * @return array|null
     */
    public static function parseComponents(string $uri, bool $expand_authority = \true, bool $validate = \true) : ?array
    {
     ...
     * @return array|null
     */
    public static function parseAuthorityComponents(string $authority, bool $validate = \true) : ?array
    {
        /** @var array|string $authority */
        if (!preg_match(self::AUTHORITY_REGEX, $authority, $authority)) {
            return null;
     ...
     * @return string
     */
    public static function buildQueryString(array $qs, ?string $prefix = null, string $separator = '&', bool $sort = \false) : string
    {
    ...
    {
        return static::buildQueryString(self::parseQueryString($query), null, '&', \true);
    }
    ...
    public static function encodeComponent(string $component, ?array $skip = null) : string
    {
        ...
        foreach (UnicodeString::walkString($component) as [$cp, $chars]) {
            if ($cp < 0x80) {
                if ($cp === 0x2d || $cp === 0x2e || $cp === 0x5f || $cp === 0x7e || $cp >= 0x41 && $cp <= 0x5a || $cp >= 0x61 && $cp <= 0x7a || $cp >= 0x30 && $cp <= 0x39 || in_array($cp, $skip, \true)) {
                    $str .= chr($cp);
                } else {
    ...
    protected static array $KNOWN_PORTS = ['ftp' => 21, 'ssh' => 22, 'telnet' => 23, 'smtp' => 25, 'tftp' => 69, 'http' => 80, 'pop' => 110, 'sftp' => 115, 'imap' => 143, 'irc' => 194, 'ldap' => 389, 'https' => 443, 'ldaps' => 636, 'telnets' => 992, 'imaps' => 993, 'ircs' => 994, 'pops' => 995];
}
