 ...
 * ============================================================================ */
namespace Matomo\Dependencies\McpServer\Opis\String;

use RuntimeException;
use OutOfBoundsException;
use Countable, ArrayAccess;
use JsonSerializable;
use Matomo\Dependencies\McpServer\Opis\String\Exception\UnicodeException;
use Matomo\Dependencies\McpServer\Opis\String\Exception\InvalidStringException;
use Matomo\Dependencies\McpServer\Opis\String\Exception\InvalidCodePointException;
class UnicodeString implements Countable, ArrayAccess, JsonSerializable
{
     ...
     * @return $this
     */
    public function ensurePrefix($text, bool $ignoreCase = \false, bool $allowPrefixOnly = \true) : self
    {
     ...
     * @return static
     */
    public function ensureSuffix($text, bool $ignoreCase = \false, bool $allowSuffixOnly = \true) : self
    {
        $text = self::resolveCodePoints($text);
        $len = count($text);
        if ($len === 0) {
    ...
    {
        return $this->doTrim($mask, \true, \true);
    }
    /**
     * @param string|self|int[]|string[] $mask
     * @return static
    ...
    {
        return $this->doTrim($mask, \true, \false);
    }
    /**
     * @param string|self|int[]|string[] $mask
     * @return static
    ...
    {
        return $this->doTrim($mask, \false, \true);
    }
    /**
     * @return static
     */
    ...
    public static function walkString(string $str) : iterable
    {
        ...
        while ($i < $length) {
            ...
            if ($ord0 < 0xe0) {
                ...
                (yield $index => [($ord0 - 0xc0) * 64 + $ord1 - 0x80, [$ord0, $ord1]]);
                continue;
            }
            if ($i === $length) {
                throw new InvalidStringException($str, $i - 1);
            ...
            if ($ord0 < 0xf0) {
                ...
                (yield $index => [($ord0 - 0xe0) * 0x1000 + ($ord1 - 0x80) * 64 + $ord2 - 0x80, [$ord0, $ord1, $ord2]]);
                continue;
            }
            if ($i === $length) {
                throw new InvalidStringException($str, $i - 1);
            ...
            if ($ord0 < 0xf5) {
                ...
                (yield $index => [($ord0 - 0xf0) * 0x40000 + ($ord1 - 0x80) * 0x1000 + ($ord2 - 0x80) * 64 + $ord3 - 0x80, [$ord0, $ord1, $ord2, $ord3]]);
                continue;
            }
            throw new InvalidStringException($str, $i - 1);
        }
    ...
    public static function getStringFromCodePoints(array $codes, int $mode = self::KEEP_CASE) : string
    {
        ...
        foreach ($codes as $code) {
            ...
            if ($code <= 0xffff) {
                $str .= chr(($code >> 12) + 0xe0) . chr(($code >> 6 & 0x3f) + 0x80) . chr(($code & 0x3f) + 0x80);
                continue;
            }
            if ($code <= 0x10ffff) {
                $str .= chr(($code >> 18) + 0xf0) . chr(($code >> 12 & 0x3f) + 0x80) . chr(($code >> 6 & 0x3f) + 0x80) . chr(($code & 0x3f) + 0x80);
            }
        }
        return $str;
    }
    ...
    private function doTrim($mask, bool $left, bool $right) : self
    {
        ...
        $codes = $this->codes;
        if ($left) {
            while (in_array($codes[0], $mask, \true)) {
                array_shift($codes);
                if (empty($codes)) {
                    return new static();
                }
        ...
        if ($right) {
            $last = count($codes) - 1;
            while (in_array($codes[$last], $mask, \true)) {
                array_pop($codes);
                if (--$last < 0) {
                    return new static();
                }
